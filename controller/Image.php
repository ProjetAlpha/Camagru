...
class Image extends ImageModel
{
    ...
    {
        if (isset($_POST['delete']) && isset($_POST['path'], $_POST['img_id']) && isAuth())
        {
            ...
            if ($splitPath[1] == "ressources" && $splitPath[2] == "images" && $splitPath[3] == $_SESSION['name'])
            {
                ...
                $this->deleteUserImg($_SESSION['name'], $path, $data['img_id']);
    ...
    public function loadImg($param = null)
    {
        if ((isset($_POST, $_POST['img']) || isset($_POST['path']) || isset($_POST['get'])) && isAuth() && isset($_SESSION['name']))
        {
            ...
            if (isset($data['img']) && isset($data['item'], $data['posX'], $data['posY'], $data['width'], $data['height'])){
                ...
                $pos  = strpos($base64, ';');
                $format = explode(':', substr($base64, 0, $pos));
                if (isset($format[1])){
                ...
                if (isset($type)){
                    ...
                    $id = $this->getUserImgId($_SESSION['name'], $js_path);
                    ...
                    imagepng($dst, $path);
                    echo json_encode([0 => ['img_path' => $js_path, 'id' => $id]]);
                }
            }
        }else {
            redirect ('/');
